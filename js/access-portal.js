(function () {
  ...
  const APP_NAME = "EHSAN FlipCards";
  const ACCESS_KEY = "ehsanFlipCardsAccessPath";
  let intendedPath = sessionStorage.getItem(ACCESS_KEY) || "";
  let pendingTeacherAccess = false;
  ...
  function addPortal() {
    ...
    portal.innerHTML = `
      <div class="access-card">
        ...
        <h1>${APP_NAME}</h1>
        <p>Choose the path that matches how you want to study or manage the app.</p>
    ...
    document.getElementById("access-teacher").addEventListener("click", () => choosePath("teacher"));
    ...
    window.requirePw = function () {
      if (firebaseAuth.currentUser) securedTeacherAccess();
      else choosePath("teacher");
    };
  }
  ...
  function choosePath(path) {
    intendedPath = path;
    pendingTeacherAccess = path === "teacher";
    ...
    document.querySelector("#cloud-auth-form p").textContent = path === "teacher"
      ...
      : "Sign in to sync progress, or create a student account.";
